...
/**
 * Provides a 'Node has image in field' condition.
 *
 * @Condition(
 *   id = "node_has_image_in_field",
 *   label = @Translation("Node has image in field")
 * )
 */
class NodeHasImageInFieldCondition extends ConditionPluginBase implements ContainerFactoryPluginInterface
{
  ...
  {
    $field_map = $this->fieldManager->getFieldMapByFieldType('image');
    if (isset($field_map['node']) && count($field_map['node'])) {
      ...
      $form['error'] = [
        '#markup' => '<p>There are no node-attached image fields in the system. This condition will return FALSE for all evaluations.</p>',
      ];
    }
